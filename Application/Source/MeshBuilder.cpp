...
float MeshBuilder::sphereX(float p, float t)
{
	return cos(Math::DegreeToRadian(p)) * cos(Math::DegreeToRadian(t));
}

float MeshBuilder::sphereY(float p, float t)
{
	return sin(Math::DegreeToRadian(p));
}

float MeshBuilder::sphereZ(float p, float t)
{
...
Mesh* MeshBuilder::GenerateText(const std::string& meshName, unsigned numRow, unsigned numCol)
{
	...
	for (unsigned row = 0; row < numRow; ++row)
	{
		...
		{
			v.pos.Set(-0.5f, -0.5f, 0.0f); v.normal.Set(0, 0, 1); v.texCoord.Set(width * (col), height * (numRow - 1 - row)); vertex_buffer_data.push_back(v); //lower left
			v.pos.Set(0.5f, -0.5f, 0.0f); v.normal.Set(0, 0, 1); v.texCoord.Set(width * (col + 1), height * (numRow - 1 - row)); vertex_buffer_data.push_back(v); //lower right
			v.pos.Set(0.5f, 0.5f, 0.0f); v.normal.Set(0, 0, 1); v.texCoord.Set(width * (col + 1), height * (numRow - row)); vertex_buffer_data.push_back(v); //top right
			v.pos.Set(-0.5f, 0.5f, 0.0f); v.normal.Set(0, 0, 1); v.texCoord.Set(width * (col), height * (numRow - row)); vertex_buffer_data.push_back(v); //top left
...
Mesh* MeshBuilder::GenerateQuad(const std::string& meshName, Color color, float size)
{
	...
	std::vector<Vertex> vertex_buffer_data;
	std::vector<unsigned> index_buffer_data;
	v.pos.Set(-0.5f * size, -0.5f * size, 0.0f); v.color = color; v.normal.Set(0, 0, 1); v.texCoord.Set(0, 0); vertex_buffer_data.push_back(v); //lower left
	v.pos.Set(0.5f * size, -0.5f * size, 0.0f); v.color = color; v.normal.Set(0, 0, 1); v.texCoord.Set(1, 0); vertex_buffer_data.push_back(v); //lower right
	v.pos.Set(0.5f * size, 0.5f * size, 0.0f); v.color = color; v.normal.Set(0, 0, 1); v.texCoord.Set(1, 1); vertex_buffer_data.push_back(v); //top right
	v.pos.Set(-0.5f * size, 0.5f * size, 0.0f); v.color = color; v.normal.Set(0, 0, 1); v.texCoord.Set(0, 1); vertex_buffer_data.push_back(v); //top left
